...
static int  *at_num;
static double *at_q;
static double *at_deut_fraction;
static int  *res_num;
...
void loadCRD( FILE *theFile, struct atom_rec *at)
{
	...
	for( int x = 0; x < psf_natoms; x++ )
	{
		...
		sscanf( buffer," %d %d %s %s %lf %lf %lf %s %d ",
			&atnum, &resnum, resname, atname, &lx, &ly, &lz, &segName, &segRes );
...
void loadPSFfromPDB( FILE *theFile )
{
	...
	at_deut_fraction = (double *)malloc( sizeof( double ) * local_nat );	
	...
	if( detected_hex )
	{
		...
		while( !feof(theFile) ) 
		{
			...
			{ 
				...
				int code = readATOMHex( buffer, &lat );
	
				if( code == 2 )
				{
					at_cnt += 1;
					// we can only handle this for TIP3P
					lat.bead = at_cnt;
					
					if( !strcasecmp( lat.resname, "TIP3" ) )
					{
						if( lat.atname[0] == 'O' )
							res_cnt++;
					}
...
//				printf("reassigned %s %s %d %d\n", lat.resname, lat.atname, lat.bead, lat.res );
				at_num[x] = lat.bead;
				res_num[x] = lat.res;
				at_q[x] = lat.charge;
...
void printPSF( FILE *thePSF, struct atom_rec *at, char **comments, int ncomments )
{
	char buffer[1024];
	...
	if( ncomments == 0 )
	{
		fprintf(thePSF, "       1 !NTITLE\n");
		fprintf(thePSF, " REMARKS generated by deuterate.exe\n");
	}
	else
	{
	 	char tbuf[256];
	 	sprintf(tbuf, "%d", ncomments );
	 
	 	for( int x = 0; x < 8 - strlen(tbuf); x++ )
	 	        fprintf(thePSF, " ");
	 	fprintf(thePSF, "%s", tbuf );   
		fprintf(thePSF, " !NTITLE\n");
		for( int x = 0; x < ncomments; x++ )
			fprintf(thePSF, " REMARKS %s\n", (comments[x] ? comments[x] : "null" ) );
	}
	fprintf(thePSF, "\n");

	 char tbuf[256];
	 sprintf(tbuf, "%d", psf_natoms );
	...
	for( int a = 0; a < psf_natoms; a++ )
	{
		...
		// segid
		sprintf( tbuf, "%s", at[a].segid );
		fprintf(thePSF, "%s", tbuf );   
		for( int x = 0; x < 5 - strlen(tbuf); x++ )
			fprintf(thePSF, " ");
		// resid
		sprintf( tbuf, "%d", at[a].res );
		fprintf(thePSF, "%s", tbuf );   
		for( int x = 0; x < 5 - strlen(tbuf); x++ )
			fprintf(thePSF, " ");
		// resname
		sprintf( tbuf, "%s", at[a].resname );
		fprintf(thePSF, "%s", tbuf );   
		for( int x = 0; x < 5 - strlen(tbuf); x++ )
			fprintf(thePSF, " ");
		// resname
		sprintf( tbuf, "%s", at[a].atname );
		fprintf(thePSF, "%s", tbuf );   
		for( int x = 0; x < 5 - strlen(tbuf); x++ )
			fprintf(thePSF, " ");
		// attype
		sprintf( tbuf, "UNK" );
		fprintf(thePSF, "%s", tbuf );   
		for( int x = 0; x < 6 - strlen(tbuf); x++ )
			fprintf(thePSF, " ");
		// charge
		sprintf( tbuf, "%lf", at[a].charge );
		for( int x = 0; x < 9 - strlen(tbuf); x++ )
			fprintf(thePSF, " ");
		fprintf(thePSF, "%s", tbuf );   
		// mass
		sprintf( tbuf, "0.0" );
		for( int x = 0; x < 14 - strlen(tbuf); x++ )
			fprintf(thePSF, " ");
		fprintf(thePSF, "%s", tbuf );   
		// unknown
		sprintf( tbuf, "0" );
		for( int x = 0; x < 12 - strlen(tbuf); x++ )
			fprintf(thePSF, " ");
		fprintf(thePSF, "%s", tbuf );   
		fprintf(thePSF,"\n");	
	}	
...
struct deuteration_comment
{
	char resName[16];
	char atDeut[16];
	double fraction;
	struct deuteration_comment *next;
};

	deuteration_comment *comments = NULL;	
	...
	for( int x = 0; x < nlines; x++ )
	{
		...
		{
			int nr = sscanf(buffer, " REMARKS deuterate %s %s fraction %lf", resName, atDeut, &to_aux ); 

			if( nr == 3 && strlen(resName) < 16 && strlen(atDeut) < 16 )
			{
				deuteration_comment *aComment = (deuteration_comment *)malloc( sizeof(deuteration_comment) );
				aComment->next = comments;
				comments = aComment;

				strcpy( aComment->resName, resName );
				strcpy( aComment->atDeut, atDeut );
				aComment->fraction = to_aux;
			}
		}	
	}
	getLine( theFile, buffer );	
...
//	printf("Reading %d atoms from the PSF.\n", local_nat );
	...
	for( int x = 0; x < local_nat; x++ )
	{
		...
		for( deuteration_comment *aComment = comments; aComment; aComment = aComment->next )
		{
			if( !strcasecmp( at_name[x], aComment->atDeut) && !strcasecmp( res_name[x], aComment->resName ) )
				at_deut_fraction[x] = aComment->fraction;
		}
...
#define TRAJ_TYPE float
	...
	for( int x = 0; x < natom; x++ )
	{
		...
		at[x].segid = (char *)malloc( sizeof(char) * (strlen(seg_name[x])+1) );
		at[x].aux = 0;
		at[x].deut_fraction = at_deut_fraction[x];
...
{
	double fac[3] = { exp(0.5*strain), exp(0.5*strain), exp(-strain) };
	*LA *= fac[0];
	*LB *= fac[1];
	*LC *= fac[2];

	if( crd_type > 0 ) // if it's fractional don't scale.
	{
		for( int a = 0; a < nat; a++ )
		{
			at[a].x *=fac[0];
...
void apply_lateral_scale( struct atom_rec *at, int nat, double *LA, double *LB, double *LC, double scale)
{
	double fac[3] = { scale, scale, 1.0 };
...
void getSegName( int a, char **out )
{
	if( a >= psf_natoms )
	{
		printf("ERROR: requested segname of an undefined atom.\n");
		exit(1);
	}

	*out = (char *)malloc( sizeof(char) * (1+strlen(seg_name[a]) ) );
	strcpy( *out, seg_name[a] );
}
...
void getResName( int a, char **out )
{
	...
	*out = (char *)malloc( sizeof(char) * (1+strlen(res_name[a]) ) );
	strcpy( *out, res_name[a] );
}
...
void getAtName( int a, char **out )
{
	...
	*out = (char *)malloc( sizeof(char) * (1+strlen(at_name[a]) ) );
	strcpy( *out, at_name[a] );
}
