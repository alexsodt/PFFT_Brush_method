...
int readATOM( char *str, struct atom_rec *atrec )
{
	int return_code = 0; // okay
	char buffer[256];
	...
	for( int s = 0; s < 5; s++ )
	{
		if( (buffer[s] >= 'a' && buffer[s] <= 'z') || (buffer[s] >= 'A' && buffer[s] <= 'Z') )
		{
...
int readATOMHex( char *str, struct atom_rec *atrec )
{
	int return_code = 1;
...
//	int atN = atoi(buffer);
	...
	for( int digit = 4; digit >= 0; digit-- )
	{
		if( buffer[digit] >= '0' && buffer[digit] <= '9' )
			val += (buffer[digit]-'0')*mult;
		else if( toupper(buffer[digit]) >= 'A' && toupper(buffer[digit]) <= 'F' )
			val += (toupper(buffer[digit])-'A' + 10)*mult;
		mult *= 16;
	}	
